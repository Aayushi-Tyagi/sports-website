...
if ( ! function_exists( 'beetan_body_classes' ) ) {
	...
	function beetan_body_classes( $classes ) {
		...
		if ( get_theme_mod( 'float_header', false ) ) {
		...
		// Add cart page class
		if ( beetan_is_woocommerce_active() && is_cart() ) {
			$classes[] = 'cart-' . get_theme_mod( 'shop_cart_layout', 'layout-1' );
			
			// Add cart total sticky class
			if ( get_theme_mod( 'sticky_cart_totals' ) && 'layout-2' === get_theme_mod( 'shop_cart_layout', 'layout-1' ) ) {
				$classes[] = 'sticky-cart-totals';
			}
			
			// Add cart auto update class
			if ( get_theme_mod( 'cart_auto_update', true ) ) {
