...
if ( ! function_exists( 'kite_maybe_print_elementor_header') ) {
	function kite_maybe_print_elementor_header() {
		...
		if ( ( kite_opt( 'is_header_build_with_elementor', false) && ! empty( $elementor_header_ID ) ) || ! empty( $custom_header) ) {
			...
			// check if site is multi language site get the translated version of header template
			if ( function_exists( 'pll_get_post' ) ) {
				$elementor_header_ID = pll_get_post( $elementor_header_ID );	   
			}

			if ( function_exists( 'wpml_object_id_filter' ) ) {
				$elementor_header_ID = wpml_object_id_filter( $elementor_header_ID, get_post_type( $elementor_header_ID ), true );
			}
	...
	function kite_maybe_print_elementor_footer() {
		if ( ! function_exists( 'elementor_theme_do_location' ) || ! elementor_theme_do_location( 'footer' ) ) {
			...
			if ( ( kite_opt( 'is_footer_build_with_elementor', false) && ! empty( $elementor_footer_ID ) ) || ! empty( $custom_footer) ) {
				...
				// check if site is multi language site get the translated version of footer template
				if ( function_exists( 'pll_get_post' ) ) {
                	$elementor_footer_ID = pll_get_post( $elementor_footer_ID );	   
                }

				if ( function_exists( 'wpml_object_id_filter' ) ) {
					$elementor_footer_ID = wpml_object_id_filter( $elementor_footer_ID, get_post_type( $elementor_footer_ID ) );
				}
...
if ( ! function_exists( 'kite_body_class_utility' ) ) {
	...
	function kite_body_class_utility( $classes ) {
		...
		if ( kite_is_blog() ) {
			if ( kite_opt( 'blog_has_fixed_sidebar_style', false ) ) {
				$classes[] = 'has-fixed-sidebar';
			}
		}
...
if ( ! function_exists( 'kite_popup_newsletter_content' ) ) {
	function kite_popup_newsletter_content() {
		...
		global $post;
		$maintenance_page_id = kite_opt( 'maintenance_page', '');
		if ( ! is_null( $post ) && kite_opt( 'maintenance_mode', true ) && !empty( $maintenance_page_id ) && $post->ID == $maintenance_page_id ) {
			return;
		}
